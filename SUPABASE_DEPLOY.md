...
| 4 | `supabase/migrations/20260910130000_referral_settlement_wallet_notifications.sql` | **Referral settlement, wallet sync + referrer notification** (4 verified bugs fixed): booking points/referral settle ab INSERT par bhi fire hota hai (pehle sirf status UPDATE par), `reward_wallets` ab points ledger ke saath sync + backfill hota hai, referrer ko `referral_qualified` notification jaata hai (purane paid referrals ka backfill bhi), aur ₹100+ QR payment se referral settle hota hai | Migration #3 ke baad chalao (idempotent, koi data drop nahi) |
| 5 | `supabase/migrations/20260910140000_profile_phone_avatar.sql` | **Profile contact persistence** — `profiles.phone` column + index, auth metadata se backfill, aur `auth.users` metadata update par profile row auto-sync (phone + full_name) | Optional but recommended — signup ka mobile number profile row me save karne ke liye (idempotent) |
| 6 | `supabase/policies/notifications.sql`, `supabase/policies/user_locations.sql` | Reference snippets (in tables/policies ki documentation) | Inka content `setup.sql` me already included hai — **skip** |
...
- **Invite/referral links live karne hain** → `20260910120000_referral_codes_invite_links.sql` bhi chalao (SQL Editor ya `db push`). Iske bina `profiles.referral_code` column nahi hota, isliye `https://<host>/invite?code=NX-…` ka code backend par resolve nahi hota (app local attribution par chalti rehti hai, cross-device counting nahi hoti).
- **Invite link ke 4 layers hain** — SQL migration ke alawa: (1) server `/invite` ko 302 karta hai → `/customer/signup?ref=…` (`server/inviteRedirects.ts`; static host par `public/_redirects`), (2) `index.html` ka pre-React capture code ko stash karta hai, (3) `VITE_APP_URL` share-link ka origin decide karta hai — set na karne par current host use hota hai, (4) `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` ke bina `/api/referrals/accept` **503** deta hai, matlab referrer ke cloud counters nahi badenge. Poora walkthrough: `DEPLOYMENT.md` §6.
- **Deploy ke baad check karo:** `npm run verify:invite -- --base https://<your-host> --code NX-VIJAY634` — har layer alag report karta hai (302 target, code retain, `?code=`→`?ref=` normalisation, resolve 200/404/503).
- **Referral rewards/points sahi count karne hain** → `20260910130000_referral_settlement_wallet_notifications.sql` chalao. Iske bina: directly-confirmed bookings par points/referral settle nahi hote, `reward_wallets` empty rehta hai, aur referrer ko koi notification nahi jaata.
- **Order matters:** `setup.sql` (baseline) → `20260908…` → `20260910120000` → `20260910130000` → `20260910140000`. Sab idempotent hain, to dobara chalana safe hai.
